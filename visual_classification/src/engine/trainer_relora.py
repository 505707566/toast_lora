...
class Trainer_relora():
    ...
    def forward_one_batch(self, inputs, targets, is_train):
        ...
        def forward_hook(name, module, input, output):
            print(f"Forward hook triggered for {name}")

        # param1 = self.model.enc.blocks._modules['0'].attn.lora1
        # param1.register_forward_hook(partial(forward_hook, 'lora1'))

        # param2 = self.model.enc.blocks._modules['0'].attn.lora2
        # param2.register_forward_hook(partial(forward_hook, 'lora2'))

        # param3 = self.model.enc.blocks._modules['0'].attn.lora3
        # param3.register_forward_hook(partial(forward_hook, 'lora3'))
    ...
    def train_classifier(self, train_loader, val_loader, test_loader, save_prediction = False, save_folder = None, save_best_test = True):
        """
        ...
        # epoch_cycles = 3 
        # total_epoch=total_epoch*epoch_cycles
            # 先冻结所有的LoRA层
        # for param in self.model.block.attn.lora1.parameters():
        #     param.requires_grad = False
        # for param in self.model.block.attn.lora2.parameters():
        #     param.requires_grad = False
        ...
        for epoch in range(total_epoch):
            ...
            # if (epoch + 1) % self.cfg.SOLVER.TOTAL_EPOCH == 0:
